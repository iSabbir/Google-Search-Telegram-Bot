...
async def search(message, bot):
    ...
    if search_type == 'image' and user_id not in admin_ids:
        if on_cooldown(user_id):
            await message.reply("You can use the /image command once every 10 minutes. Please wait. join @bdbots")
            return
        else:
    ...
    if query:
        ...
        try:
            ...
            search_results_message = await message.reply("<b>⚠️ Google SafeSearch is on, so no results , join @bdbots </b>", parse_mode=types.ParseMode.HTML)
